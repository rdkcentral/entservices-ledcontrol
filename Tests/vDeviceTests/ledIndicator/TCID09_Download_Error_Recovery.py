"""
/**
 * @file TCID09_Download_Error_Recovery.py
 * @brief L2 LED indicator functional testcase.
 *
 * @testcase TCID09_Download_Error_Recovery
 * @details TCID09_test_l2_led_download_error_recovery validates the 'download error recovery' LED behavior using JSON-RPC and/or vComponent state simulation.
 *
 * @precondition
...
def run_test():
    Start_Profiling = time.perf_counter()  # Profiling started - track execution time for software download error recovery sequence (ACTIVE -> DOWNLOAD_ERROR -> ACTIVE)
    log_info("TCID09_test_l2_led_download_error_recovery - Scenario: Software Download Error Recovery")
    log_info("  ACTIVE -> DOWNLOAD_ERROR -> ACTIVE")
    steps = [
        ("SetState_ACTIVE.yaml",         "ACTIVE"),
        ("SetState_SOFTWARE_DOWNLOAD_ERROR.yaml", "DOWNLOAD_ERROR"),
        ("SetState_ACTIVE.yaml",          "ACTIVE"),
    ]
    for yaml_file, expected in steps:
        ...
        if not _post(yaml_file):
            ...
            log_error("TCID09_Download_Error_Recovery Failed ❌")
            return False
        time.sleep(2)
    ...
    Elapsed_Profile = time.perf_counter() - Start_Profiling  # Profiling ends - compute total time taken to complete download error recovery flow
    log_success("TCID09_Download_Error_Recovery Passed ✅" + f" time consumed: {Elapsed_Profile:.3f}s")
    return True
